...
import sqlancer.hazelcast.HazelcastSchema.HazelcastIndex;
import sqlancer.hazelcast.HazelcastSchema.HazelcastTable;

import java.util.List;
import java.util.stream.Collectors;

public final class HazelcastIndexGenerator {

    private HazelcastIndexGenerator() {
    }
    ...
    public static SQLQueryAdapter generate(HazelcastGlobalState globalState) {
        ...
        // }
        HazelcastTable randomTable = globalState.getSchema().getRandomTable(t -> !t.isView()); // TODO: materialized
                                                                                              // views
        ...
        if (method == IndexType.HASH) {
            ...
            for (int i = 0; i < Randomly.smallNumber() + 1; i++) {
                ...
                if (Randomly.getBoolean()) {
                    ...
                    sb.append("(");
                    HazelcastExpression expression = HazelcastExpressionGenerator.generateExpression(globalState,
                            randomTable.getColumns());
                    sb.append(HazelcastVisitor.asString(expression));
                    sb.append(")");
                }
        ...
        if (Randomly.getBoolean()) {
            sb.append(" WHERE ");
            HazelcastExpression expr = new HazelcastExpressionGenerator(globalState).setColumns(randomTable.getColumns())
                    .setGlobalState(globalState).generateExpression(HazelcastDataType.BOOLEAN);
    ...
    private static String getNewIndexName(HazelcastTable randomTable) {
        List<HazelcastIndex> indexes = randomTable.getIndexes();
        int indexI = 0;
        while (true) {
